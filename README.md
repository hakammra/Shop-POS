...
Run `supabase/sql/074_inventory_conditions_reservations_consignment_dashboard.sql` after migration 073. It adds Stock Condition Transfer documents for moving the same units between Sellable, Damaged, and Checking without cashflow or cost changes; Reservation Orders that reserve stock and optionally record a customer advance; and Consignment Intake/Return documents for stock owned by another person. Consignment payout is used as sale-time cost, so the owner payable is created only when the item sells and gross profit is the shop commission. The Dashboard also gains date-range sales, COGS, gross-profit, cash, purchase, outstanding, order, recent-document, and low-stock views with a mobile layout.
...
Run `supabase/sql/077_round_automatic_prices_and_reset_empty_pos_customer.sql` after migration 076. Selling prices automatically recalculated from a changed average cost and the existing markup are then rounded upward to the next LKR 50 (`610 → 650`, `640 → 650`, and `650 → 650`). The Products form uses the same rule when Cost or Markup is changed; a selling price entered manually is not altered. After a page refresh or PIN login, an empty POS bill also returns to Walk-in Customer, while any bill containing items retains its selected customer and ongoing work.
