-- v51: More relevant, punctuation-tolerant Tech Assistant product search.
-- Run once after 050_admin_start_fresh.sql, then redeploy the tech-assistant Edge Function.
...
begin
  ...
  with raw_terms as (
    select regexp_replace(lower(token.term_value), '^[^[:alnum:]]+|[^[:alnum:]]+$', '', 'g') as term
    from regexp_split_to_table(clean_query, E'\\s+') as token(term_value)
  ), term_candidates as (
    select regexp_replace(term, '[^[:alnum:]]+', '', 'g') as term from raw_terms
    union
    select token.term_value as term from regexp_split_to_table(normalized_query, E'\\s+') as token(term_value)
  ), terms as (
    ...
    where char_length(term) >= 2
      and term not in (
        ...
        'lookup','shop','store','pos','product','products','item','items','sell','carry'
      )
  ), prepared as (
    select p.*,
...
comment on function public.assistant_search_context_v45(text) is
  'Permission-gated, read-only assistant lookup with weighted product-code, barcode, name, model and category ranking.';
